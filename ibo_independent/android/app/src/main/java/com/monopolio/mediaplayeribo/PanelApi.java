...
public final class PanelApi {
    ...
    public static String register(DeviceIdentity d) throws Exception {
        ...
        j.put("device_key", d.deviceKey); j.put("mac", d.mac); j.put("android_id", d.androidId);
        j.put("manufacturer", d.manufacturer); j.put("model", d.model); j.put("app_version", BuildConfig.VERSION_NAME);
        JSONObject r = request("POST", BuildConfig.PANEL_API_URL + "?action=register", j, null);
    ...
    public static Config config(DeviceIdentity d, String token) throws Exception {
        String u = BuildConfig.PANEL_API_URL + "?action=config&device_key=" + java.net.URLEncoder.encode(d.deviceKey, "UTF-8");
        JSONObject r = request("GET", u, null, token);
        if (!r.optBoolean("ok")) throw new Exception(r.optString("error", "config_failed"));
        Config c = new Config(); c.status = r.optString("status", "pending");
        JSONObject x = r.optJSONObject("config");
        if (x != null) {
            c.playlistName = x.optString("playlist_name", "TV DIGITAL");
            c.server = x.optString("server", ""); c.username = x.optString("username", ""); c.password = x.optString("password", "");
        }
        return c;
    }
    ...
    private static JSONObject request(String method, String urlString, JSONObject body, String token) throws Exception {
        ...
        c.setRequestProperty("Accept", "application/json"); c.setRequestProperty("X-App-Key", BuildConfig.PANEL_APP_KEY);
