...
public class BotService extends Service {
    ...
    private volatile boolean running; private WuzApiManager wuz; private LocalWebhookServer webhook; private PanelClient panel; private RuleEngine engine;

    @Override public void onCreate(){
        super.onCreate();createChannel();startForeground(14,notification("Bot iniciando…"));running=true;new Thread(this::main,"mr-main").start();
    }

    private void main(){
        try{
            wuz=new WuzApiManager(this);engine=new RuleEngine(this,wuz);webhook=new LocalWebhookServer(engine);webhook.start();wuz.start();panel=new PanelClient(this);
            // Inicia conexión en segundo plano sin bloquear el servicio para siempre.
            new Thread(()->{try{wuz.connect();}catch(Exception e){android.util.Log.w("BotService","Conexión inicial pendiente: "+e.getMessage());}},"wa-connect").start();
            ...
            while(running){
                try{
                    ...
                    if(panel.linked()){
                        ...
                        JSONArray rules=sync.optJSONArray("rules");if(rules!=null)Prefs.put(this,"rules_json",rules.toString());
    ...
    private synchronized void restartEngine(){
        new Thread(()->{
            try{
                ...
                notifyText("Motor reiniciado · reconectando");
            }catch(Exception e){
                android.util.Log.e("BotService","restart",e);
                notifyText("Error reiniciando motor");
            }
        },"mr-restart").start();
    }
